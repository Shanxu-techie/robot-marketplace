...
async function main() {
  ...
  const insertedRobots = await db
    ...
    .values([
...

      {
        name: 'OfficeGuide O1',
        slug: 'officeguide-o1',
        shortDescription: 'Reception and visitor assistance robot for offices.',
        fullDescription:
  ...
  const robotMap = Object.fromEntries(insertedRobots.map((r) => [r.slug, r.id]))
  ...
  const insertedUsers = await db
    .insert(users)
    .values({
      clerkUserId: 'user_test12345678901234567890',
      email: 'abc@gmail.com',
      role: 'buyer',
    })
    .returning()
  console.log(`${insertedUsers.length} users`)
  const seedUserId = insertedUsers[0].id
